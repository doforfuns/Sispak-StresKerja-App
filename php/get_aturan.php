...
if (isset($input['id_penyakit'])) {
    ...
    $q_penyakit = "SELECT id_penyakit, kode_penyakit, nama_penyakit FROM penyakit WHERE id_penyakit='$id_penyakit'";
    ...
    if (mysqli_num_rows($result_penyakit) > 0) {
        ...
        while ($row_gejala = mysqli_fetch_assoc($result_gejala)) {
            $gejala[] = $row_gejala['kode_gejala'] . ' - ' . $row_gejala['nama_gejala'];
        ...
        $response["nama_penyakit"] = $penyakit['kode_penyakit'] . ' - ' . $penyakit['nama_penyakit'];
